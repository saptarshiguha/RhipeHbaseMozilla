...
names attribute equal to =columnfamily:columnname= . Each sublist
entry has two elements (as is usual in RHIPE) - the key and value. The
values are raw bytes (and if the raw bytes correspond to Java strings,
they can be converted using the R function =rawToChar=). The keys are
also raw bytes.
